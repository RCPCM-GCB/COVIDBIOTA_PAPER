...
meta_cov_spain <- read_delim("../DATA/SPAIN/meta.tsv", 
    "\t", escape_double = FALSE, trim_ws = TRUE)
meta_cov_spain <- as.data.frame(meta_cov_spain)
row.names(meta_cov_spain) <- meta_cov_spain$Run
meta_cov_spain$country <- 'Spain'
meta_cov_spain$city <- 'Spain'
...
ps_genus
ps_tip <- speedyseq::tip_glom(ps, h=0.2)
ps_tip
```
...
annotation_colors = list(
  ...
  CommunityType = c('1'='green','2'='red','3'='blue','4'='grey','5'='purple','6'='orange', '7'='black', '8'='brown', '9'='cyan'),
  city=c("Moscow" ='red',   "NNovgorod" = 'blue', "Irkutsk" = 'green' ,  "Kazan" = 'pink', 'Spain' = 'yellow'   ),
  Severity = c('A'='green','B'='yellow','C'='red','D'='black')
)
...
#                                      # meta_for_heatmap$KT,
#                                      cc$ASV7__Firmicutes.g__Staphylococcus,
#                                      cc$ASV2__Bacteroidota.g__Prevotella),]
# 
# # reorder with hclust 
# # https://rdrr.io/cran/textshape/src/R/cluster_matrix.R
# hc2  <- stats::hclust(stats::dist(ordered_counts), method = 'ward.D2')
# ordered_counts <- ordered_counts[hc2$order,]
...
# ps_obj <- prune_samples(sample_sums(ps_obj) > 500, ps_obj)
...
plot_ordination(ps_obj, ordination, color='city', shape='country') + theme(aspect.ratio=1) + ggtitle('NMDS bray')  + geom_point(size=3)  
# + geom_text(aes( label = mgx_specimen), size = 3, vjust = 1.5, alpha=0.3)
```
...
# saveRDS(fit, '../../RESULTS/dmm_fit_genus_naRmT_prev_0.1__detection5_1to10.rds')
fit <- readRDS('../../RESULTS/dmm_fit_genus_naRmT_prev_0.1__detection5_1to10.rds')
```
